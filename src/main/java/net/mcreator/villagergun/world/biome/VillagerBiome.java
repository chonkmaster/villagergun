...
package net.mcreator.villagergun.world.biome;
...
public class VillagerBiome extends VillagergunModElements.ModElement {
	@ObjectHolder("villagergun:villager")
	public static final CustomBiome biome = null;
	public VillagerBiome(VillagergunModElements instance) {
		super(instance, 13);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(VillagerBlockBlock.block.getDefaultState(),
							VillagerStoneBlock.block.getDefaultState(), VillagerStoneBlock.block.getDefaultState())));
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == VillagerBlockBlock.block.getDefaultState().getBlock()
							|| ground == VillagerStoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == VillagerBlockBlock.block.getDefaultState().getBlock()
									|| ground2 == VillagerStoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), VillagerStoneBlock.block.getDefaultState(), bbox);
